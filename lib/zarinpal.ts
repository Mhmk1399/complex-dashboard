  ...
  static async requestPayment(
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `ZarinPal request failed: ${result.errors.message} (Code: ${result.errors.code})`
        );
      }

      if (result.data && result.data.code !== 100) {
        throw new Error(
          `ZarinPal request failed: ${result.data.message} (Code: ${result.data.code})`
        );
      }
  ...
  static async verifyPayment(
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `ZarinPal verify failed: ${result.errors.message} (Code: ${result.errors.code})`
        );
      }
