...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      "Merchant ID:",
      process.env.ZARINPAL_MERCHANT_ID ? "[SET]" : "[NOT SET]"
    );
    ...
    if (result.data?.code === 100 || result.data?.code === 101) {
      ...
      console.log(
        "Shipping address from pending order:",
        pendingOrder.shippingAddress
      );
      const storeId = getStoreId(request);

      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { authority, paymentAmount, expiresAt, ...orderFields } =
        pendingOrder as Record<string, unknown>;
      const orderData = {
        ...orderFields,
        status: "processing", // Use valid enum value
        paymentStatus: "completed",
        paymentAuthority: authority as string,
      ...
      console.log(
        "Payment verification failed. Zarinpal code:",
        result.data?.code
      );
      return NextResponse.json(
        {
