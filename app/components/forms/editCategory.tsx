...
const EditCategory = () => {
  ...
  const removeFromParent = async (childId: string) => {
    ...
    const parentCategory = categories.find(cat => cat.children.includes(childId));
    
    if (parentCategory) {
      try {
        const updatedChildren = parentCategory.children.filter(id => id !== childId);
  ...
  const renderCategory = (category: Category, level: number = 0) => {
    ...
    const childCategories = categories.filter(cat => category.children.includes(cat._id));
                  ...
                  <div className="flex items-center  gap-2 flex-col sm:flex-row w-full sm:w-auto">
                    <div className="flex gap-2 w-full sm:w-auto lg:justify-start justify-center ">
                      <motion.button
                        ...
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.95 }}
                        onClick={() => selectedParentId && addToParent(selectedParentId, category._id)}
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="p-4 sm:p-8 bg-gradient-to-br from-[#0077b6] to-[#023e8a] rounded-2xl shadow-2xl mx-2 sm:mx-6 lg:max-w-6xl lg:mx-auto mt-10"
      dir="rtl"
    >
      ...
      <div className="space-y-4 bg-white/10 p-3 sm:p-6 rounded-xl backdrop-blur-lg">
        {categories
          .filter(category => !categories.some(cat => cat.children.includes(category._id)))
